...
function grant_permission() {
  PGPASSWORD=$ADMIN_PASSWORD psql -d $ADMIN_DB -h $DB_HOST -p $DB_PORT -U $ADMIN_USER -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_ADMIN"
  PGPASSWORD=$ADMIN_PASSWORD psql -d $ADMIN_DB -h $DB_HOST -p $DB_PORT -U $ADMIN_USER -c "ALTER DATABASE $DB_NAME OWNER TO $DB_ADMIN"
  PGPASSWORD=$ADMIN_PASSWORD psql -d $ADMIN_DB -h $DB_HOST -p $DB_PORT -U $ADMIN_USER -c "ALTER SCHEMA public OWNER TO $DB_ADMIN"
  PGPASSWORD=$ADMIN_PASSWORD psql -d $ADMIN_DB -h $DB_HOST -p $DB_PORT -U $ADMIN_USER -c "ALTER USER $DB_ADMIN WITH CREATEDB;"
}

function dump() {
  mkdir -p "$DUMP_DIR"
  PGPASSWORD=$DB_PASSWORD pg_dump -d $ADMIN_DB -U "$DB_ADMIN" -h "$DB_HOST" -p "$DB_PORT" -F t "$DB_NAME" > "$DUMP_FILE"
}

function drop_db() {
  PGPASSWORD=$ADMIN_PASSWORD psql -d $ADMIN_DB -h $DB_HOST -p $DB_PORT -U $ADMIN_USER -c "DROP DATABASE IF EXISTS $DB_NAME"
}
