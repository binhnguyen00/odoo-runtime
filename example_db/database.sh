...
function init_admin() {
  ADMIN_EXISTS=$(PGPASSWORD=$ADMIN_PASSWORD psql -h $DB_HOST -p $DB_PORT -U $ADMIN_USER -tAc "SELECT 1 FROM pg_roles WHERE rolname='$DB_ADMIN'")
  if [ "$ADMIN_EXISTS" == "1" ]; then
    echo "USER $DB_ADMIN ALREADY EXISTS. SKIPPING CREATION."
  else
    PGPASSWORD=$ADMIN_PASSWORD psql -h $DB_HOST -p $DB_PORT -U $ADMIN_USER -c "CREATE USER $DB_ADMIN WITH PASSWORD '$DB_PASSWORD';"
  fi
  grant_permission
}
